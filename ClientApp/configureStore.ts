...
const connection = new SignalR.HubConnection('http://localhost:61550/SignalRHub');

export function signalRInvokeMiddleware(store: any) {
    return (next: any) => async (action: any) => {
        switch(action.type){
            case "SIGNALR_TEST1":
                connection.invoke('Test1');
                break;
            case "SIGNALR_TEST1":
                connection.invoke('Test1');
                break;
...
export function signalRRegisterCommands(store: any, callback: Function){
    connection.on('Test1', data => {
        store.dispatch({ type: 'TEST1' });
        console.log("Test1 is dispatched.");
    })
    connection.on('Test2', data => {
        store.dispatch({ type: 'TEST2' });
        console.log("Test2 is dispatched.");
    })
